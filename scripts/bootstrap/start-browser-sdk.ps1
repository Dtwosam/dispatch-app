...
if ($env:ARC_SERVER_WALLET_ADDRESS -and -not $env:PLATFORM_AGENT_OWNER_WALLET) {
  $envDefaults.PLATFORM_AGENT_OWNER_WALLET = $env:ARC_SERVER_WALLET_ADDRESS
}

$services = @(
  @{ Name = "evaluator"; Command = "npm run dev:evaluator"; Port = 4030; ExtraEnv = @{} },
  @{ Name = "router"; Command = "npm run dev:router"; Port = 4020; ExtraEnv = @{} },
  @{ Name = "adapter"; Command = "npm run dev:adapter"; Port = 4010; ExtraEnv = @{} },
  @{ Name = "web"; Command = "npm run dev:web"; Port = 3005; ExtraEnv = @{ PORT = "3005" } }
)
...
foreach ($service in $services) {
  ...
  $mergedEnv = @{}
  foreach ($pair in $envDefaults.GetEnumerator()) {
    $mergedEnv[$pair.Key] = $pair.Value
  }
  foreach ($pair in $service.ExtraEnv.GetEnumerator()) {
    $mergedEnv[$pair.Key] = $pair.Value
  }
  $envBlock = ($mergedEnv.GetEnumerator() | ForEach-Object {
...
Write-Host "Started Arc browser-wallet local stack:"
$started | ForEach-Object { Write-Host " - $($_.name) on port $($_.port) (PID $($_.pid))" }
Write-Host "Open http://localhost:3005, switch to Arc Testnet, and connect your browser wallet."
