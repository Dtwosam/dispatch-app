...
| Circle Agent Stack | Circle Agent Stack | https://developers.circle.com/agent-stack | Supports agent wallets, agent nanopayments, x402 service discovery, and Circle Skills. | Nano architecture, future backend/API, local readiness script | Readiness-only | Local script checks CLI/tooling only; do not claim agent wallets or Circle nanopayments are implemented until repo proves it. |
| Circle Wallets | Circle docs index | https://developers.circle.com/llms.txt | Potential wallet model for agent-managed budgets and payouts. | Future Nano backend/payment services, local readiness mapping | Readiness-only | Current app uses browser wallet path; do not claim Circle Wallet custody or balances are active. |
...
| x402 | Circle Gateway Nanopayments | https://developers.circle.com/gateway/nanopayments | Provides a pay-per-request model for sources/tools that respond with payment requirements. | Nano source/tool endpoint and future API access | Partially implemented | Nano has an unpaid HTTP 402 payment-required source endpoint; do not claim buyer payment, paid content access, or Gateway settlement until implemented and verified. |
