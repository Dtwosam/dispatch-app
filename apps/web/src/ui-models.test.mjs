...
test("Nano Dispatch agent selector exposes only real built-in agents without fake metrics", () => {
  ...
  assert.equal(selector.helper, "Choose who should request source unlocks for this goal.");
...
test("Nano selected built-in agent stays in Goal state before budget and names source/result copy safely", () => {
  ...
  assert.equal(selector.selectedStateCopy, "Research Brief will request source unlocks if this goal needs paid context.");
...
test("Nano public stats render before wallet-gated console in the Nano page template", () => {
  ...
  const statsSection = appSource.slice(statsIndex, statsSectionEnd);
